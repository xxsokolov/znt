...
@telegram_router.post("/zbx_tpda/tg/sendMessage", status_code=status.HTTP_200_OK,
                      response_model=schemas.telegram.ZabbixTPDA, summary="Отправить сообщение в Telegram",
                      description='Zabbix Media Type: ZNT TPDA')
async def telegram_send_message(request: Request, schema: schemas.telegram.ZabbixTPDA,
...
@telegram_router.post("/zbx_service/tg/sendMessage", status_code=status.HTTP_200_OK,
                      response_model=schemas.telegram.ZabbixTPDA, summary="Отправить сообщение в Telegram",
                      description='Zabbix Media Type: ZNT Service')
async def telegram_send_message(request: Request, schema: schemas.telegram.ZabbixService,
                                db: Session = Depends(get_db)):
    try:
        json_ = await request.json()
    except json.decoder.JSONDecodeError:
