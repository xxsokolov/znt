...
class ZNT:
    ...
    def __create_message(self):
        message_header = html.escape(self.send.message.header.format_map(FailSafeDict(json.loads(config.get('zabbix', 'zabbix_status_emoji_map')))))
        if config.get('zabbix', 'body_messages_cut_symbol') and len(self.send.message.body) > int(config.get('zabbix', 'body_messages_max_symbol')):
            truncated = True
            self.logger.info("Message truncated to {} characters".format(config.get('zabbix', 'body_messages_max_symbol')))
        else:
            truncated = False
        body = '{} <a href="{}">...</a>'.format(
            html.escape(self.send.message.body)[:config.get('zabbix', 'body_messages_max_symbol')],
        ...
        links = ' '.join(self.links) if config.get('zabbix', 'body_messages_url') and len(self.links) != 0 else ''

        tags = ' '.join(self.tags) if config.get('zabbix', 'body_messages_tags') and len(self.tags) != 0 else ''

        tags_settings = ' '.join(self.zntsettings['tags']) if config.get('zabbix', 'body_messages_tags') and len(self.zntsettings['tags']) != 0 else ''

        mentions = ' '.join(self.mentions) if not isinstance(self.mentions, bool) and config.get('zabbix', 'body_messages_mentions_settings') and len(self.mentions) != 0 else ''

        self.message = config.get('zabbix', 'body_messages').format(subject=message_header,
                                            body='\n\n'+body if body else '',
                                            links='\n'+links if links else '',
                                            tags='\n\n'+tags if tags else '',
    ...
    def __watermark_text(self):
        try:
            ...
            for chart in self.chart_png:
                ...
                if img.height < int(config.get('zabbix', 'watermark_minimal_height')):
                    self.logger.info(
                        "Cannot set watermark text, img height {} (min. {})".format(img.height, config.get('zabbix', 'watermark_minimal_height')))
                    return False

                font = ImageFont.load_default()
                line_width, line_height = font.getsize(config.get('zabbix', 'watermark_label'))

                txt = Image.new('RGBA', (line_width, line_height))

                from ast import literal_eval
                ImageDraw.Draw(txt).text((0, 0), config.get('zabbix', 'watermark_label'), fill=literal_eval(config.get('zabbix', 'watermark_fill')), font=font)

                txt = txt.rotate(int(config.get('zabbix', 'watermark_rotate')), resample=Image.BICUBIC, expand=True)

                img_size = img.crop().size
                size = (img_size[0] - txt.size[0] - 10, img_size[1] - txt.size[1] - 5)
                img.paste(literal_eval(config.get('zabbix', 'watermark_text_color')), box=size, mask=txt)
    ...
    def __create_chart(self):
        if (self.options.graphs and config.get('zabbix', 'zabbix_graph')) and not self.settings_no_graph:
            settings_dash = next(
                (x for x in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')] if config.get('zabbix', 'trigger_settings_tag_grafana_dash') in x), None)
            if settings_dash:
    ...
    def __create_links(self):
        ...
        self.links.append(self.__create_links_list(
            _bool=True if self.options.triggerlinks and config.get('zabbix', 'body_messages_url_notes') else False,
            url=self.macros.triggerurl,
            _type=config.get('zabbix', 'body_messages_url_emoji_notes')))
        # graph url
        for item_id in list(set([x for x in self.macros.itemid.split()])):
            if re.findall(r"\d+", item_id):
                items_link = self.__create_links_list(
                    _bool=True if self.options.graphlinks and config.get('zabbix', 'body_messages_url_graphs') else False,
                    url=config.get('zabbix', 'zabbix_graph_link').format(zabbix_server=config.get('zabbix', 'url'),
                                                                         itemid=item_id,
                                                                         range_time=self.options.graphs_period),
                    _type=config.get('zabbix', 'body_messages_url_emoji_graphs')
                )
                self.links.append(items_link) if items_link else None
        # host url
        self.links.append(self.__create_links_list(
            _bool=True if self.options.hostlinks and config.get('zabbix', 'body_messages_url_host') else False,
            url=config.get('zabbix', 'zabbix_host_link').format(zabbix_server=config.get('zabbix', 'url'),
                                                                host=self.macros.hostname),
            _type=config.get('zabbix', 'body_messages_url_emoji_host')))
        # ack url
        self.links.append(self.__create_links_list(
            _bool=True if self.options.acklinks and config.get('zabbix', 'body_messages_url_ack') else False,
            url=config.get('zabbix', 'zabbix_ack_link').format(zabbix_server=config.get('zabbix', 'url'),
                                                               eventid=self.macros.eventid),
            _type=config.get('zabbix', 'body_messages_url_emoji_ack')))
        # event url
        self.links.append(self.__create_links_list(
            _bool=True if self.options.eventlinks and config.get('zabbix', 'body_messages_url_event') else False,
            url=config.get('zabbix', 'zabbix_event_link').format(zabbix_server=config.get('zabbix', 'url'),
                                                                 eventid=self.macros.eventid,
                                                                 triggerid=self.macros.triggerid),
            _type=config.get('zabbix', 'body_messages_url_emoji_event')))
        return

    def __create_tags(self):
        # self.__create_tags_list()
        event = self.__create_tags_list(_bool=True if self.options.eventtag and config.get('zabbix', 'body_messages_tags_event') else False,
                                        tag=self.macros.eventtags, _type=None)
        eventid = self.__create_tags_list(
            _bool=True if self.options.eventtag and config.get('zabbix', 'body_messages_tags_eventid') else False,
            tag=self.macros.eventid, _type=config.get('zabbix', 'body_messages_tags_prefix_eventid'))
        itemid = self.__create_tags_list(
            _bool=True if self.options.itemidtag and config.get('zabbix', 'body_messages_tags_itemid') else False,
            tag=' '.join([item_id for item_id in self.macros.itemid.split() if re.findall(r"\d+", item_id)]),
            _type=config.get('zabbix', 'body_messages_tags_prefix_itemid'))
        triggerid = self.__create_tags_list(
            _bool=True if self.options.triggeridtag and config.get('zabbix', 'body_messages_tags_triggerid') else False,
            tag=self.macros.triggerid, _type=config.get('zabbix', 'body_messages_tags_prefix_triggerid'))
        actionid = self.__create_tags_list(
            _bool=True if self.options.actionidtag and config.get('zabbix', 'body_messages_tags_actionid') else False,
            tag=self.macros.actionid, _type=config.get('zabbix', 'body_messages_tags_prefix_actionid'))
        hostid = self.__create_tags_list(
            _bool=True if self.options.hostidtag and config.get('zabbix', 'body_messages_tags_hostid') else False,
            tag=self.macros.hostid, _type=config.get('zabbix', 'body_messages_tags_prefix_hostid'))
    ...
    def __create_graph_name(self):
        self.chart_name = config.get('zabbix', 'body_messages_title').format(title=self.send.charts.title,
                    ...
                    for tags in tag.split(', '):
                        if tags:
                            if tags.find(':') != -1:
                                ...
                                if tag != config.get('zabbix', 'trigger_settings_tag') and tag != config.get('zabbix', 'trigger_info_mentions_tag'):
                            ...
                            tags_list.append(config.get('zabbix', 'body_messages_tags_no'))
                else:
                    tags_list.append(config.get('zabbix', 'body_messages_tags_no'))
            else:
                return False
        ...
        try:
            if self.options.zntsettingstag and self.macros.eventtags:
                for tags in self.macros.eventtags.split(', '):
                    if tags.find(':') != -1:
                        tag, value = re.split(r':+', tags, maxsplit=1)
                        if tag == config.get('zabbix', 'trigger_settings_tag'):
                            tags_list.append('#{tag}_{value}'.format(
                                tag=config.get('zabbix', 'trigger_settings_tag') if config.get('zabbix', 'trigger_settings_tag') else re.sub(r"\W+", "_", tag),
                                value=re.sub(r"\W+", "_", value)))
            ...
            self.zntsettings = {'tags': tags_list, config.get('zabbix', 'trigger_settings_tag'): settings_list}
            self.logger.info("Найдены тэги настроек: {}: {}".format(
                config.get('zabbix', 'trigger_settings_tag'),
                ', '.join(self.zntsettings[config.get('zabbix', 'trigger_settings_tag')])))
        ...
        try:
            if self.options.zntmentions and self.macros.eventtags:
                ...
                for tags in self.macros.eventtags.split(', '):
                    if tags.find(':') != -1:
                        tag, value = re.split(r':+', tags, maxsplit=1)
                        if tag == config.get('zabbix', 'trigger_info_mentions_tag'):
        ...
        try:
            if _bool:
                if url and (re.search(r'\w', url)):
                    return config.get('zabbix', 'body_messages_url_template').format(url=url, icon=_type)
                else:
                    return config.get('zabbix', 'body_messages_url_emoji_no_url')
            elif url_list:
                return url_list
            else:
                return False
    ...
    def __handling_settings(self):
        # no_alert
        if config.get('zabbix', 'trigger_settings_tag_no_alert') in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')]:
            self.logger.warning("Отправка сообщения отменена: {}:'{}'".format(
                config.get('zabbix', 'trigger_settings_tag'), config.get('zabbix', 'trigger_settings_tag_no_alert')))
            self.settings_no_alert = True
            exit(0)

        # 'no_graph'
        if config.get('zabbix', 'trigger_settings_tag_no_graph') in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')]:
            self.logger.warning("Отправка сообщения без графиков: {}:'{}'".format(
                config.get('zabbix', 'trigger_settings_tag'), config.get('zabbix', 'trigger_settings_tag_no_graph')))
            self.settings_no_graph = True

        # 'not_notify'
        if config.get('zabbix', 'trigger_settings_tag_not_notify') in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')]:
            self.logger.info("Отправка сообщения без оповещения: {}:'{}'".format(
                config.get('zabbix', 'trigger_settings_tag'), config.get('zabbix', 'trigger_settings_tag_not_notify')))
    ...
    def __settings_chart_period(self):
        if isinstance(self.zntsettings, dict) and not all(
                settings.find(config.get('zabbix', 'trigger_settings_tag_graph_period')) and len(settings) > 0 for settings in
                self.zntsettings[config.get('zabbix', 'trigger_settings_tag')]):
            try:
                chart_period_raw = [i if i.find(config.get('zabbix', 'trigger_settings_tag_graph_period')) == 0 else False for i in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')]][0]
                chart_period = int(chart_period_raw.split('=')[1])
            except Exception as err:
                self.logger.error("Exception occurred: {}:{}, {}".format(
                    config.get('zabbix', 'trigger_settings_tag'), chart_period_raw, err),
            ...
            self.chart_period = config.get('zabbix', 'zabbix_graph_period_default')
    ...
    def __init_bot(self):
        # _default = 'production'
        settings_raw = None
        settings_bot = next((x for x in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')] if config.get('zabbix', 'trigger_settings_tag_bot') in x), None)
        settings_bot_group = next((x for x in self.zntsettings[config.get('zabbix', 'trigger_settings_tag')] if config.get('zabbix', 'trigger_settings_tag_bot_group') in x), None)
        ...
        if settings_bot:  # Если указано имя бота в тэге znts:bot=@username
            try:
                ...
                self.logger.error("Exception occurred: {}:{}, {}".format(
                    config.get('zabbix', 'trigger_settings_tag'), settings_raw, err),
                    exc_info=config.get('core', 'exc_info')), exit(1)
            else:
                # Ищем имя бота отвечающего требованиям Telegram
                if re.search("^(?=.{5,35}$)@[a-zA-Z0-9_]+(?:bot|Bot)", znts_bot):
                    self.logger.info("Отправка сообщения через бота: {}: {}'".format(config.get('zabbix', 'trigger_settings_tag'), znts_bot))
            ...
            try:
                ...
                self.logger.info("Отправка сообщения через бота в группе: {}: {}".format(config.get('zabbix', 'trigger_settings_tag'), znts_bot_group))
                list_priority = []
